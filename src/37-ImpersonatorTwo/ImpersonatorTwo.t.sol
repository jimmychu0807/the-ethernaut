...
contract ImpersonatorTwoTest is Test {
    ...
    function testSolveImpersonatorTwo() public view {
        ...
        bytes32 z1 = instance.hash_message(
            string(abi.encodePacked("admin1", address(0xADa4aFfe581d1A31d7F75E1c5a3A98b2D4C40f68)))
        );

        address recoveredAddr = ECDSA.recover(z0, abi.encodePacked(R, S0, V0));
        address recoveredAddr1 = ECDSA.recover(z1, abi.encodePacked(R, S1, V1));
        require(recoveredAddr == recoveredAddr1, "two recovered addresses are different!");
        ...
        for (uint256 i = 0; i < sks.length; i++) {
            ...
            if (addr == recoveredAddr) {
                console.log("sk: %s", vm.toString(bytes32(sk)));
    ...
    function _printSkAndAddr(uint256[] memory sks, bool printSk, bool printAddr) internal pure {
        for (uint256 i = 0; i < sks.length; i++) {
            uint256 sk = sks[i];
            address addr = vm.addr(sk);
            if (printSk) console.log("sk: %s", vm.toString(bytes32(sk)));
