...
// Libraries
import {Lib_MerkleTrie as MerkleTrie} from "../helpers/lib/trie/Lib_MerkleTrie.sol";
import {Lib_RLPReader as RLPReader} from "../helpers/lib/rlp/Lib_RLPReader.sol";
import {Lib_RLPWriter as RLPWriter} from "../helpers/lib/rlp/Lib_RLPWriter.sol";
// import "../helpers/lib/rlp/RLPErrors.sol";
...
contract MerkleTrie_Harness {
    ...
    function exposed_get(bytes memory _key, bytes[] memory _proof, bytes32 _root) public pure returns (bytes memory) {
        (bool exists, bytes memory val) = MerkleTrie.get(_key, _proof.writeList(), _root);
        require(exists, "value not found in merkle tree");
        return val;
    }
}
...
contract MerkleTrie_Get_Test is MerkleTrie_TestInit {
    ...
    function test_get_validProof1_succeeds() external pure {
        ...
        bytes[] memory rlpEncodedProof = new bytes[](3);
        rlpEncodedProof[0] = proof[0].writeBytes();
        rlpEncodedProof[1] = proof[1].writeBytes();
        rlpEncodedProof[2] = proof[2].writeBytes();

        // (bool exists, bytes memory getVal) = MerkleTrie.get(key, rlpEncodedProof.writeList(), root);
        // assertTrue(exists);
        // assertEq(val, getVal);

        assertTrue(MerkleTrie.verifyInclusionProof(key, val, rlpEncodedProof.writeList(), root));
    }
    ...
    function test_get_validProof3_succeeds() external pure {
        ...
        bytes[] memory rlpEncodedProof = new bytes[](1);
        rlpEncodedProof[0] = proof[0].writeBytes();
    ...
    //     (bool exists, bytes memory getVal) = MerkleTrie.get(key, proof.writeList(), root);
    //     assertTrue(exists);
    //     assertEq(val, getVal);
    // }
